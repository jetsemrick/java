...
public interface GroupAlertDao extends JpaRepository<GroupAlert, Long>, JpaSpecificationExecutor<GroupAlert> {
    ...
    /**
     * Demo / incident seed: find group alarms whose key is prefixed for cleanup.
     */
    @Query("select g from GroupAlert g where g.groupKey like concat(:prefix, '%')")
    List<GroupAlert> findByGroupKeyStartingWith(@Param("prefix") String prefix);

    /**
     * Demo seed cleanup: remove synthetic group alarms by key prefix.
     */
    @Modifying(clearAutomatically = true, flushAutomatically = true)
    @Query("delete from GroupAlert g where g.groupKey like concat(:prefix, '%')")
    void deleteByGroupKeyStartingWith(@Param("prefix") String prefix);
}
